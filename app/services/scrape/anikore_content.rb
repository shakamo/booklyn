...
module Scrape
  class AnikoreContent

    @max_page = 0

    def self.createAll(year, season)
      create(year, season, 'ac:tv')
      create(year, season, 'ac:movie')
    end
    
    def self.create(year, season, type)
      @max_page = 0

      for page in 1..99 do
        url = 'http://www.anikore.jp/chronicle/' + year + '/' + season + '/' + type + '/page:' + page.to_s
        document_of_anikore = Common::UrlUtils.instance.get_document(url)

        if document_of_anikore == nil then
          StandardMailer.error_mail('AnikoreContent', 'データの取得に失敗しました。url:' + url).deliver
          raise 'AnikoreContent データの取得に失敗しました。url:' + url
        end

        load_page(document_of_anikore, year, season)
    ...
    def self.load_page(document_of_anikore, year, season)

      # Maxページ数を取得する。
      document_of_anikore.css('#main > div.paginator > span').each do |node|
      ...
      # タイトルを繰り返す。
      title_list = document_of_anikore.xpath('//*[@id="main"]/div')
      ...
      document_of_anikore.xpath('//*[@id="main"]/div').each do |node|
    ...
    def self.set_title(content, node)
      ...
      content.title = Common::RegexUtils.get_title_for_anikore(title)
    end

    def self.set_category(content, node)
      title = node.css('div[1]/span[2]/a').inner_text
    ...
    def self.set_initial_by_detail_page(content)
      doc_by_detail = Common::UrlUtils.instance.get_document_for_anikore_by_detail(content.id.to_s)
    ...
    def self.set_schedule(content, node, year, season)
    ...
    def self.get_schedule(content, node)
      begin
