  ...
  class Anikore
      ...
      document.xpath('//*[@id="main"]/div').each do |node|
        if node.css('div[1]/span[2]/a').inner_text != ''
          ...
          if content_id.nil?
            ...
            fail 'AnikoreContent Anikore ' + year + ' ' + season + ' ' + 'ContentIdが取得できません。'
          end
    ...
    def set_category(content, node)
      ...
      if title.index('（テレビアニメ）')
        ...
        if content.category_id.nil?
          StandardMailer.error_mail('AnikoreContent', 'categoryが取得できません。' + title).deliver
          fail 'AnikoreContent Anikore Categoryが取得できません。' + title
        end
      end
    end
    ...
    def set_image(content, node)
      ...
      if url.nil?
        ...
        fail 'AnikoreContent Anikore ' + year + ' ' + season + ' ' + 'ImageのURLがnilです。' + content.id.to_s + '. title is' + content.title + '.'
      end
    ...
    def get_schedule(_content, node)
